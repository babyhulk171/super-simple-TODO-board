...
export function KanbanCardView({ card, dispatch, onEdit }: KanbanCardViewProps) {
  ...
  return (
    <article ref={sortable.setNodeRef} style={style} className={`kanban-card ${sortable.isDragging ? "is-dragging" : ""} ${card.completed ? "is-complete" : ""}`} onPointerDown={(event) => { event.stopPropagation(); sortable.listeners?.onPointerDown?.(event); }}>
